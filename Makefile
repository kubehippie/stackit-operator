IMG_DEFAULT ?= ghcr.io/kubehippie/stackit-operator:latest
IMG ?= $(IMG_DEFAULT)
...
manifests: controller-gen ## Generate WebhookConfiguration, ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=manager-role crd webhook paths="./api/..." paths="./cmd/..." paths="./internal/..." paths="./test/..." output:crd:artifacts:config=config/crd/bases

.PHONY: generate
generate: controller-gen ## Generate code containing DeepCopy, DeepCopyInto, and DeepCopyObject method implementations.
	$(CONTROLLER_GEN) object:headerFile="hack/boilerplate.go.txt" paths="./api/..." paths="./cmd/..." paths="./internal/..." paths="./test/..."

.PHONY: grafana
grafana: ## Generate the dashboards with kubebuilder.
...
KIND_CLUSTER ?= stackit-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name stackit-operator-builder
	$(CONTAINER_TOOL) buildx use stackit-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm stackit-operator-builder
	rm Dockerfile.cross
...
build-installer: manifests generate kustomize ## Generate a consolidated YAML with CRDs and deployment.
	...
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG_DEFAULT}
