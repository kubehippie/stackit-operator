...
var _ = Describe("PostgresUser Controller", func() {
	Context("When reconciling a resource", func() {
		...
		postgresuser := &postgresv1alpha1.PostgresUser{}
		...
		BeforeEach(func() {
			...
			err = k8sClient.Get(ctx, typeNamespacedName, postgresuser)
			if err != nil && errors.IsNotFound(err) {
				resource := &postgresv1alpha1.PostgresUser{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: resourceNamespace,
					},
		...
		It("should reconcile the resource and fail to resolve the instance ID", func() {
			...
			controllerReconciler := &PostgresUserReconciler{
				Client: k8sClient,
